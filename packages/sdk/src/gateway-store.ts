...
function getDefaultGatewayUrl(): string {
  ...
  const env = (g['process'] as { env?: Record<string, string | undefined> } | undefined)?.env

  const explicit = env?.NEXT_PUBLIC_GATEWAY_WS_URL
  if (explicit) return explicit

  if (env?.NODE_ENV === 'development') return 'ws://localhost:64737/ws'
...
export type { GatewayStore, VoiceOpusFrame, VoiceMode, ChannelState, UserState, ChatItem, Metrics, ServerListEntry, MumbleServerConfig, ContextAction, SavedCredentials }

export interface CreateGatewayStoreOptions {
  gatewayUrl?: string
  storageKey?: string
}

export function createGatewayStore(options: CreateGatewayStoreOptions = {}): UseBoundStore<StoreApi<GatewayStore>> {
  const gatewayUrl = options.gatewayUrl ?? getDefaultGatewayUrl()
  const storageKey = options.storageKey ?? 'mumble-gateway-storage'

  return create<GatewayStore>()(
  persist(
    (set, get) => {
      const uplink = {
